...
export function Setup({ auth, onDone }: { auth: AuthApi; onDone: (u: SessionUser) => void }) {
  ...
  const [f, setF] = useState({ username: '', displayName: '', password: '', again: '' });
  ...
  const create = async (e: Event) => {
    ...
    try {
      setUser(await auth.setup({ username: f.username.trim(), displayName: f.displayName.trim(), password: f.password }));
    } catch (x) {
